...
```{r}
# Remove rows with missing notes
data_clean <- data[!is.na(data$Notes), ]
print(data_clean)

# Define keywords that suggest a mismatch between FPS-R score and observer score
mismatch_keywords <- c("mischievous", "histrionic", "game", "no pain", "quietly and calmly", "did not withdraw",
                       "really hurt", "did not feel", "said owee", "dramatic personality", "scored a 10 while smiling",
                       "said owe but did not withdraw", "did not react in keeping with a score of 10")
...
linear_model <- lm(`FPS-R score` ~ Age + Sex + Practitioner + `Number of IV attempts` + `Needle gauge`, data = data)
...
# Pie chart for Observed side effects (Redness, Itching, Puffiness, pain)
```{r}
...
categorize_effects <- function(effect) {
  ...
  if (grepl("redness|pink", tolower(effect))) {
    category <- c(category, "Redness")
  }
  if (grepl("itchy|prurit", tolower(effect))) {
    category <- c(category, "Itching")
  }
  if (grepl("puffy", tolower(effect))) {
    category <- c(category, "Puffiness")
  }
  if (grepl("pain", tolower(effect))) {
    category <- c(category, "Pain")
  }
