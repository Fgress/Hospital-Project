...
draw_pie_chart <- function(all_categories, title) {
  ...
  pie(category_counts,
      main = title,
      col = rep(c("green", "red"), length.out = length(category_counts)),
      labels = labels,
      cex = 1.0)
}
```
...
#### Linear regression by two groups

```{r, echo=TRUE}
control <- subset(data_subset, Randomization == 1)
study <- subset(data_subset, Randomization == 2)

lm_model_c <- lm(`FPS-R score` ~ Age + `Number of IV attempts` + `Needle gauge`, data = control)
lm_model_s <- lm(`FPS-R score` ~ Age + `Number of IV attempts` + `Needle gauge`, data = study)
summary(lm_model_c)
summary(lm_model_s)
...
draw_pie_chart <- function(all_categories, title) {
  ...
  # Create a pie chart with percentages
  colors_hex <- c("#283618", "#bc6c25")
  alpha_value <- 0.7
  colors_with_alpha <- sapply(colors_hex, function(col) {
  col_rgb <- col2rgb(col) / 255
  rgb(col_rgb[1], col_rgb[2], col_rgb[3], alpha = alpha_value)}, USE.NAMES = FALSE)
  colors_with_alpha <- rep(colors_with_alpha, length.out = length(category_counts))
  
  pie(category_counts,
      main = title,
      col = colors_with_alpha,
      labels = labels,
      cex = 1.0)
}
```
